...
extern "C" { // Use extern "C" to ensure C linkage for functions called from C
    void RunCppApplication() {
    	...
    	/* Initialize Embedded Message System */
    	MessageResult_t msg_result = MessagePool_Init();
    	if (msg_result != MSG_RESULT_OK) {
    		printf("ERROR: Failed to initialize MessagePool! Result: %d\n", msg_result);
    		// Error handler aquí si es necesario
    	} else {
    		printf("SUCCESS: MessagePool initialized successfully\n");
    		
    		// Ejecutar tests del sistema de mensajes
    		printf("Running MessageSystem tests...\n");
    		MessageSystem_BasicTest();
    		MessageSystem_SensorExample();
    		MessageSystem_CppWrapperExample();
    	}
