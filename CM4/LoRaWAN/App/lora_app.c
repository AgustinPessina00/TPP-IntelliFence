...
/* USER CODE BEGIN PV */
// Funciones externas de loraTask
extern uint8_t loraTaskGetPayload(uint8_t *buffer, uint8_t maxSize);
extern bool loraTaskHasNewData(void);
extern void loraTaskOnJoinSuccess(void);
/* USER CODE END PV */
...
static void OnRxData(LmHandlerAppData_t *appData, LmHandlerRxParams_t *params)
{
  ...
  if (appData != NULL) {
    APP_LOG(TS_OFF, VLEVEL_M, "RX Data on port %d, size: %d\r\n", 
            appData->Port, appData->BufferSize);
    // Aquí puedes procesar comandos downlink si es necesario
  }
...
static void SendTxData(void)
{
  ...
  uint8_t txBuffer[242]; // Buffer máximo según LORAWAN_APP_DATA_BUFFER_MAX_SIZE
  
  // Obtener el payload desde loraTask
...
static void OnJoinRequest(LmHandlerJoinParams_t *joinParams)
{
  ...
  if (joinParams->Status == LORAMAC_HANDLER_SUCCESS) {
    APP_LOG(TS_OFF, VLEVEL_M, "JOIN SUCCESS!\r\n");
    loraTaskOnJoinSuccess();
  } else {
    APP_LOG(TS_OFF, VLEVEL_M, "JOIN FAILED, retrying...\r\n");
  }
