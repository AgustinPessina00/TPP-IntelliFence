...
bool Lsm6dso::writeRegister(uint8_t reg, uint8_t value) {
	uint8_t data = value;
  return HAL_I2C_Mem_Write(this->hi2c, this->i2cAddr, reg, I2C_MEMADD_SIZE_8BIT, &data, 1, 10) == HAL_OK;
}
...
int32_t Lsm6dso::lsm6dso_read_reg(uint8_t reg, uint8_t *data, uint16_t len){
  return HAL_I2C_Mem_Read(this->hi2c, this->i2cAddr, reg, I2C_MEMADD_SIZE_8BIT, data, len, 10);
}
...
int32_t Lsm6dso::lsm6dso_acceleration_raw_get(int16_t *val) {
  ...
  ret = HAL_I2C_Mem_Read(this->hi2c, this->i2cAddr, REG_OUTX_L_A, I2C_MEMADD_SIZE_8BIT, buff, 6, 10);
  if (ret != HAL_OK)
    return -1;
