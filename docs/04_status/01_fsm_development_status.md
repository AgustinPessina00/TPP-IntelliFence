...
- **Integración con Cow y Fence**:
  ```cpp
  // TODO: Descomentar cuando Cow esté listo
  // fsmTaskParams *fsmParams = static_cast<fsmTaskParams *>(argument);
  // fsmParams->cow->updatePosition(...)
  ```
  
  ✅ **Fence ya está implementado** (sin std::vector, arrays estáticos):
  - `bool saveVertices(const Vertex* v, uint8_t count)` - Array estático (max 20 vértices)
  - `void createLimits()` - Sin push_back, usando índices
  - `void clearVertices()` - Limpieza con memset
  - Zero dynamic allocation, MISRA-C compliant
